...
## Decision 43 — Recurring Events (Wire Existing `generateRecurringChildren`; Weekly/Monthly, Regenerate)

**Date decided:** 2026-06-14
**Decision:** Expose recurring-event generation via a new `POST /events/{event}/recurrence` endpoint. The rule is `{ frequency: weekly|monthly, occurrences: 1–52 }` (matching the existing `EventService::generateRecurringChildren`). Re-running **regenerates**: it deletes upcoming **non-completed** child occurrences and recreates the series, always preserving completed occurrences. Only a top-level event (no `parent_event_id`) can start a series; children link back to the parent. Gated by `events.manage` (read via `events.view`).
**Rationale:** GAPS.md §4 "Recurring events UI" was tagged frontend-only, but verification showed the service method `generateRecurringChildren` existed yet was **never called by any route**, and `recurrence_rule` wasn't validated anywhere — so it needed backend wiring, not just UI. The `recurrence_rule` (jsonb) and self-referential `parent_event_id` columns already exist, so **no migration or dependency** is required. Weekly/Monthly + occurrences matches the service as-is (no new `match` arms). Regenerate (chosen over generate-once) lets staff fix a mis-entered cadence without manually deleting occurrences; scoping the delete to *non-completed* children protects historical/billed events.
**Implementation:**
- `EventService::applyRecurrence(Event $parent, array $rule): Collection` — in a transaction: delete `parent_event_id = parent` children where `status != completed`, persist `recurrence_rule` on the parent, call `generateRecurringChildren`, return the fresh children ordered by date. (Cancel already cascades to future non-completed children.)
- `EventController::recurrence` — `events.manage`, tenant `abort_if`, `abort_if(parent_event_id !== null, 422)`, validates `frequency` (`in:weekly,monthly`) + `occurrences` (`integer|min:1|max:52`); returns `{ data: parent, children }`. `show()` now also eager-loads `parentEvent` (id/title/organiser/date) and ordered `childEvents`.
- Route `POST /events/{event}/recurrence` in the `plan.feature:events` group.
- `/web`: `Event` type + MST model gain `recurrence_rule` (typed `RecurrenceRule`), `parent_event`, `child_events`; `EventService.generateRecurrence` + `EventStore.generateRecurrence` (re-fetches via `show` to populate children). Event-detail "Recurrence" tab: parents get a frequency/occurrences form (Generate/Regenerate) + a clickable occurrences list; children get a "part of a series" note linking to the parent. en-US keys under `events.recurrence_*`.

---

## Decision 44 — Inventory List Pagination (`meta.total` Shape, Load-More Accumulation)

**Date decided:** 2026-06-14
**Decision:** The inventory **items**, **suppliers**, **waste-logs**, and per-item **stock-movements** list endpoints are paginated using the existing `ApiResponse::paginated()` envelope — `{data: items, meta: {current_page, last_page, per_page, total}}` — the same convention the customers/orders/events lists already use. `per_page` defaults to 20, capped at 100. The `/web` consumers request `per_page=100` and **accumulate** pages client-side (a "Load more" button appends the next page without dropping loaded rows), so the shared `DataTable`'s existing client-side paging keeps working with no UX regression.
**Rationale:** GAPS.md §4 cross-cutting — these four endpoints returned a bare `{data: get()}` array (no pagination), and four pre-existing inventory feature tests asserted `data.total` against that array (and so were failing). Two response shapes were possible: `{data: <paginator>}` (`data.total`, items at `data.data` — matches the stale tests verbatim but diverges from the rest of the app) vs. the codebase-standard `{data: items, meta:{total}}`. **Chosen: `meta.total`** for one app-wide pagination convention; the four stale test assertions were updated `data.total → meta.total` accordingly (they now pass). Movements switched from an unbounded `get()` to a bounded `per_page=100` fetch (a performance improvement) with no load-more (it is a bounded detail sub-view, not a top-level list).
**Implementation:**
- API: `InventoryItemController::index`/`movements`, `SupplierController::index`, `WasteLogController::index` → `ApiResponse::paginated($query->paginate(min($request->integer('per_page', 20), 100)))`.
- `/web` `InventoryService`: the four list methods return the full `{data, meta}` (`ListMeta` exported); `LIST_PER_PAGE = 100`. `getMovements` keeps returning just `.data` (no UI change).
- `InventoryStore`: `itemsMeta`/`suppliersMeta`/`wasteLogsMeta` + saved filter params + `isLoadingMore*` flags; `hasMoreItems`/`hasMoreSuppliers`/`hasMoreWaste` views; `loadMoreItems`/`loadMoreSuppliers`/`loadMoreWasteLogs` actions (page-1 fetch replaces/reconciles; load-more upserts/concats and never deletes). Items map reconciles (delete-missing) on a fresh fetch only.
- `/web` items / suppliers / waste-logs pages render a `common.load_more` button below the `DataTable` when `hasMore*`.
- Tests: 4 api assertions updated to `meta.total` (InventoryItemTest ×2 incl. movements, SupplierAndPoTest, WasteLogTest — all green); web `InventoryStore.test.ts` mocks wrapped in a `paged()` helper (22 pass).

---

## Decision 45 — Global Header Search (Grouped, Per-Source Permission-Gated)

**Date decided:** 2026-06-14
**Decision:** A single `GET /v1/search?q=` endpoint powers the staff-app header search, querying five sources — **customers, orders, reservations, events, menu items** — and returning results **grouped by type, top 5 per group**, ranked most-recent/alphabetical within each group. Each source is searched **only if the caller holds that module's view permission** (`customers.view_basic`, `orders.view`, `reservations.view`, `events.view`, `menu.view`); a user never sees rows from a module they cannot otherwise view. No new permission slug, migration, or dependency.
**Rationale:** GAPS.md §4 cross-cutting — the header search input was static (no backend). Scope (entities, shape, permission model) was confirmed with product via AskUserQuestion: grouped-top-N over a single ranked list (predictable, no cross-type ranking to tune), and per-source gating over one coarse `search.global` slug (respects the app's per-module gating; avoids surfacing entities a user can't view). All five models are tenant-scoped via their `HasTenantScope` global scope, so the search auto-scopes to the resolved tenant with no manual filter.
**Implementation:**
- `App\Http\Controllers\Api\V1\SearchController::index` — min 2 chars (else empty groups), `%term%` LIKE per source, `Gate::allows()` guard per group, `limit(5)`. Search columns: customers → `customer_tenant_profiles.loyalty_number` + related `customer_profiles.{phone,first_name,last_name,email}` (returns the **tenant-profile id**, matching the `/customers/$customerId` detail route); orders → `order_ref`/`customer_name`/`customer_phone`; reservations → `confirmation_code`/`guest_name`; events → `title`/`organiser_{name,phone,email}`; menu items → `name`/`sku`. Each row is `{id, label, sub}`; the **frontend** maps `type`→detail route (backend stays decoupled from routing). Route `GET /search` registered in the authenticated tenant group (no `plan.feature`).
- `/web` `GlobalSearch.tsx` (replaces the static `<SearchInput>` in `Header.tsx`): debounced (300 ms) `apiClient.get('/search')`, grouped dropdown (heading per non-empty group + up to 5 rows), outside-click close, select → `navigate` to the per-type detail route (`/customers/$customerId`, `/orders/$orderId`, `/reservations/$reservationId`, `/events/$eventId`, `/menu/items/$itemId`). Transient local `useState` (report-page idiom, not MST). en-US `common.search_{searching,no_results,group_*}` keys.

---

## Decision 46 — Metric Alert Thresholds (Dedicated Table, Branch-Local Completed-Day Window, In-App + Email)

**Date decided:** 2026-06-14
**Decision:** Staff can define **metric alerts** — a metric + comparison operator + threshold, optionally scoped to one branch (null = tenant-wide) — that fire an in-app notification (+ email to configured recipients) when the metric breaches the threshold. Alerts are stored in a new dedicated **`metric_alerts`** table (not the settings registry — alerts carry mutable per-row runtime state, `last_triggered_at`/`last_value`, that would be race-prone inside a shared JSONB blob the user also edits). Gated by the **existing** `analytics.configure_alerts` permission slug (already in `config/permissions.php`; granted to owner + restaurant_admin). No new slug.
**Supported metrics (10, all computable from existing sources):** `gross_revenue`, `net_revenue`, `order_count`, `cover_count`, `avg_order_value` (from `analytics_daily_revenue`); `food_cost_pct`, `cogs`, `waste_cost` (from `CogsReportService`, movement-derived); `labour_cost_pct` (attendance × `users.employment.hourly_rate` ÷ gross revenue, replicating the Decision 37 staff-report computation); `revpash` (from `RevpashReportService`). Operators: `gt`, `lt`, `gte`, `lte`.
**Evaluation window:** because both the revenue family and RevPASH read from the pre-aggregated `analytics_daily_revenue` (populated only by the 02:00 daily rollup for the prior day), intraday "today" is not available — so alerts evaluate over **completed days** in the tenant primary-branch timezone (Decision 26): per-alert `window` = `yesterday` (default) | `last_7_days` | `last_30_days`. Tenant-wide alerts (`branch_id` null) aggregate across all branches; the services already accept a null branch.
**Cadence + throttle:** `EvaluateMetricAlertsCommand` runs **every 15 minutes** (`withoutOverlapping`), iterating enabled alerts. On each run it computes the metric and updates `last_value`; it notifies only when the value **breaches** AND `last_triggered_at` is null or older than the start of today (branch-local) — i.e. **at most once per day per alert** while breaching (the 15-min cadence keeps it responsive after the 02:00 rollup and picks up newly-created alerts promptly; yesterday's data is static so re-evaluation is idempotent).
**Delivery:** in-app via `NotificationService::send` to the alert's `created_by` staff user (deterministic recipient — avoids a permission-graph broadcast query); email via a queued `MetricAlertBreachedMail` to the alert's `email_recipients` list (may be empty → in-app only). SMS is not used.
**Implementation:**
- Migration `create_metric_alerts_table` (Schema::create Blueprint idiom per `customer_merges`): `id` uuid pk, `tenant_id` FK→tenants cascade, `branch_id` nullable FK→branches cascade, `metric` varchar(40), `operator` varchar(4), `threshold` decimal(14,4), `window` varchar(20) default `yesterday`, `enabled` bool default true, `email_recipients` jsonb nullable, `last_triggered_at` timestampTz nullable, `last_value` decimal(14,4) nullable, `created_by` FK→users, timestamps, index `[tenant_id, enabled]`. Brand-new table (never deployed) → single create file, `migrate:fresh`.
- `MetricAlert` model (`HasTenantScope` + `HasUuids`); `MetricMetric`/`MetricOperator`/`AlertWindow` enums; `MetricAlertEvaluator` service (metric → value for tenant/branch/window). `EvaluateMetricAlertsCommand` (scheduled 15-min). `MetricAlertBreachedMail` Mailable.
- `MetricAlertController` (index/store/update/destroy) gated `analytics.configure_alerts`; `StoreMetricAlertRequest`/`UpdateMetricAlertRequest` (validate metric/operator/window enums, branch belongs to tenant, recipients are emails); `MetricAlertResource`. Routes under the authenticated tenant group.
- `/web`: `/analytics/alerts` page + AnalyticsNav tab (self-gated on `analytics.configure_alerts`) — CRUD over alerts via a new `AlertService`/store-or-local-state, metric/operator/window `Select`s, threshold `NumberInput`, optional branch `Select`, email recipients, enable `Switch`. en-US `analytics.alerts_*` keys.

---

## Decision 47 — Custom Report Builder = Saved Report Configurations (Settings Registry, Reuses Existing Reports)

**Date decided:** 2026-06-14
**Decision:** The "custom report builder" (GAPS §4 line 68) is implemented as a **saved report configurations** layer over the existing analytics reports — **not** a generic dynamic query engine (that would be large and over-engineered for MVP). A saved report is a named, reusable combination of `{report_type, branch, date-range}` that staff can one-click **run** (navigates to the matching report page pre-filtered) and **export** (the existing Wave-8 `ExportButton` on each report page). Stored as a tenant **setting** key `reporting.saved_reports` (JSONB list) via `SettingsService`, mirroring `reporting.scheduled_reports` (Decision: no migration). Gated by the **existing** `analytics.custom_reports` permission slug (already in `config/permissions.php`; granted to owner + restaurant_admin). No new slug.
**Saved-report shape:** `{ id (uuid), name, report_type (revenue|dishes|customers|inventory|staff|tax|cogs|revpash), branch_id|null, date_mode (preset|custom), preset (last_7_days|last_30_days|last_90_days|this_month|last_month)|null, from|null, to|null }`.
**Rationale:** every report page already supports branch + free date range + presets (Wave 7 `DateRangePresets`) + async export (Wave 8 `ExportButton`). A saved-configuration layer delivers the user-facing value of a "builder" (define once, re-run) by composing what exists, with zero new query/aggregation code and no schema. A full column/dimension/aggregation builder is recorded as deferred.
**Implementation:**
- `SavedReportController` (`index` GET / `update` PUT — full-list replace, exactly like `ScheduledReportController`), gated `analytics.custom_reports`; `UpdateSavedReportsRequest` validates each entry's `report_type`/`date_mode`/`preset` enums and branch ownership; setting key `reporting.saved_reports`. Routes `GET/PUT /analytics/saved-reports`.
- `/web`: `/analytics/saved-reports` page + AnalyticsNav tab (self-gated on `analytics.custom_reports`) — list saved reports with **Run** (→ navigate to `/analytics/{report_type}` with `branch`/`from`/`to` resolved from the preset) and add/edit/delete via a form (name, report_type `Select`, branch `Select`, date_mode + preset/custom DatePickers). en-US `analytics.saved_*` keys.

## Decision 48 — Surface Delivery Financials (`orders.platform_commission` + `order_payments.refund_reason`)

**Date decided:** 2026-06-14
**Context:** Orphaned-data audit (memory `orphaned-data-audit`, plan `.claude/plans/2026-06-14-orphaned-data-surfacing.md` Wave 2). Two stored-but-invisible fields written by integration/refund flows but exposed in no Resource and rendered nowhere in `/web`.
**Decision A — `orders.platform_commission` (set by `ProcessUberEatsOrderJob`/`ProcessWoltOrderJob` = `payment.merchant_fee_amount`):** surfaced **per-order and aggregated** (audit Option B).
- Per order: `OrderResource` returns `platform_commission` and computed `net_revenue` (= `total − platform_commission`, `null` when commission null). `/web` order detail renders "Commission paid" + "Net to restaurant" rows for delivery sources only.
- Aggregate: a **`platform_commission` column was added to the pre-aggregated `analytics_daily_revenue` table** (merged into the create migration `…000052`, dev-only — re-run `migrate:fresh`; **not** a live-sum — chosen over the plan's default after review). `AggregateDailyRevenueJob` sums it per `(tenant,branch,date,channel)`. The revenue report (`ReportController@revenue`) returns `commission_paid` (`SUM(platform_commission)`) and `net_delivery_revenue` (delivery gross − commission, channels `uber_eats`/`wolt`) in `totals`. `/web` revenue dashboard shows both as cards. Distinct from Decision 39 commission *rates* — this is platform actuals.
**Decision B — `order_payments.refund_reason` (set by `RefundEngine`/`PaymentService`, both Stripe and manual paths):** surfaced **persistently** via an order payments list (audit/plan chose persistence over response-only). New `PaymentResource` (method/amount/status/refund_amount/refund_reason/created_at); `OrderController@show` eager-loads `payments`; `OrderResource` exposes `payments` via `whenLoaded`; `PaymentController@refund` additionally returns `refund_reason` from the refunded payment. `/web` order detail renders a Payments section showing the reason on refunded rows; the refund panel re-fetches the order on success so the row appears without reload.
**Gates (all pre-existing):** `orders.view` (order detail), `orders.view_payment`/`orders.refund` (payments/refund), `analytics.revenue_branch` (revenue report).
**i18n (en-US):** `orders.platform_commission`, `orders.net_to_restaurant`, `orders.refund_reason`, `orders.payments_heading`, `orders.refunded_label`; `analytics.commission_paid`, `analytics.net_delivery_revenue`.
...
**Rationale:** Surfaces the orphaned `events.custom_menu` column flagged in the orphaned-data audit. Brainstormed shape/pricing/spend/render/permission choices were confirmed with the user (priced line items + optional menu link; per-row mode with single price; display-only estimate; own tab; update-only; full EventResource).
**Implementation:**
- **API:** new `UpdateEventRequest` (nested `custom_menu.*` rules; `withValidator` enforces "quantity required for per_item" because Laravel `required_if` does not resolve the `*` wildcard to the row index; `menu_item_id` uses a **tenant-scoped** `Rule::exists` honoring tenant isolation). New `EventService::normalizeCustomMenu()` (generates row UUIDs, nulls quantity for per_head, coerces types). New `App\Http\Resources\Events\EventResource` outputting the full event shape + `custom_menu` + computed `custom_menu_subtotal`; **all** event-returning `EventController` methods (index/show/store/update + every transition/action + recurrence) now flow through it so the single MST `_upsertEvent` always receives one consistent shape (`index` keeps pagination meta hand-built, the `orders()` idiom; `ApiResponse` import dropped from the controller). Run-sheet PDF (`run-sheet.blade.php`) renders a Custom Menu block (data already carried into `run_sheet` JSONB by `buildRunSheet`). en-US `events.custom_menu_*` + `events.estimated_event_value` keys in `lang/en-US/web.php`.
- **/web:** `CustomMenuItem` type + `Event.custom_menu`/`custom_menu_subtotal` + `UpdateEventPayload.custom_menu`; `EventStore` model fields + `_upsertEvent` mapping; new `CustomMenuTab` on the event detail `TabBar` (local working copy, inline add/edit/remove via design-system `SegmentedControl`/`Input`/`NumberInput`/`Combobox`/`IconButton`, running subtotal, single Save → existing `updateEvent`); display-only Financials lines (custom menu subtotal + estimated event value). Edit controls gated on `events.manage`.

---

## Decision 50 — Native iOS & Android Employee Apps (Fully Native, 100% Staff Parity)

**Date decided:** 2026-06-16
**Context:** Employees can only work from the `/web` browser app today — unsuitable for on-the-floor, kitchen, and receiving-dock work where a phone is the natural device. Planned in `.claude/plans/let-s-plan-to-build-iridescent-candle.md` (program-level spec).
**Decision:** Build **two fully native apps** — **iOS (SwiftUI)** and **Android (Jetpack Compose)** — giving employees **100% parity** with every action in `/web` (staff guard only; **no** `/admin` platform-admin actions, and **not** the consumer `/portal`/`/landing` flows). No React Native / Capacitor / Flutter; no UI/store reuse from `/web`. UI is **platform-idiomatic** (Apple HIG on iOS, Material 3 on Android), not a pixel port of the web design system. Adds native capabilities: **push notifications, camera/barcode scanning, biometric unlock, and offline support.**
**Scope is a multi-quarter program, not one drop** (~112 web screens × 2 platforms + backend prerequisites + offline sync). Delivered in sequenced waves; each wave is its **own** brainstorm → spec → plan → implement cycle. Foundation waves (0–2) must land and be proven on a vertical slice before module waves scale out.
**Architecture (shared across both platforms):** typed API client over `/api/v1` injecting `Authorization: Bearer <token>` + `X-Branch-Id`, central 401 → clear session → login (mirrors `web/src/services/api.ts`); two-step OTP auth (`/auth/staff/login` → `/auth/staff/otp/send` → `/auth/staff/otp/verify`); token in Keychain (iOS) / EncryptedSharedPreferences+Keystore (Android) behind a biometric gate; permission helper mirroring `authStore.can('slug')` (UI gating is UX only, API re-validates); per-module repository over a local DB (SwiftData/Core Data; Room) with a write **outbox** for offline replay (default last-write-wins; **86/food-safety events never auto-replay** — manual confirm); real-time via Reverb's Pusher protocol (PusherSwift / pusher-java-client) authorizing private channels through `/broadcasting/auth`; push via APNs (iOS) + FCM (Android).
**Backend work is in scope (`cheflogik-api`, Wave 0 prerequisite):** `device_tokens` table + register/unregister endpoints; `PushProviderInterface` (FCM + APNs, mirroring `SmsProviderInterface`) wired into notification dispatch over RabbitMQ; mobile token abilities + a one-shot OTP-verify payload (token + permissions + branch list); `updated_since`/cursor delta-sync on hot list endpoints (orders, menu/86, reservations, inventory, notifications, messages) with idempotent bulk-replay for the outbox.
**Repos:** new sibling repos `cheflogik-ios` (SwiftUI) and `cheflogik-android` (Jetpack Compose), git-ignored by the docs repo like `/web` and `/admin`.
**Wave sequence:** W0 backend prereqs → W1 native foundations (both platforms) → **W2 gate: Orders + KDS + Notifications** vertical slice → W3 floor ops (reservations, tables, messages, dashboard) → W4 kitchen & stock (inventory, menu) → W5 people (staff, shifts, attendance, leave, payroll, profile, roles) → W6 guests & growth (customers, campaigns, events) → W7 admin & insight (15 analytics dashboards, settings, branches, landing CMS).
**Rationale:** User chose fully-native + 100% parity + all native capabilities + platform-idiomatic + backend-in-scope via AskUserQuestion during planning. Native gives the best on-device experience for kitchen/floor staff; the foundation-first gate retires real-time/push/offline/food-safety risk once rather than 112 times.

**Amendment (2026-06-18) — Wave 2 gate scope:** The W2 gate is **Orders + Notifications** (not "Orders + KDS + Notifications"). **KDS is removed from the native apps entirely** — not deferred. The kitchen display is a stationary in-kitchen screen, not a phone workflow; the 30-second allergen-acknowledgement food-safety rule is therefore **KDS-only and stays web-only**. On mobile, food-safety is carried solely by the orders **outbox carve-out**: cancels with `reason_code = out_of_stock` (and any stock-driven mutation) are flagged `requiresManualReplay` and **never auto-replay** — a manager confirms them. Wave sequence updated: …→ W2 gate (Orders + Notifications) → W3 floor ops…; KDS drops out of the module waves. Confirmed Wave-2 sub-decisions: **real-time via Pusher client libs** (PusherSwift on iOS, pusher-java-client on Android) authorizing private channels through `/broadcasting/auth`; **offline-queueable writes limited to order status-transition + cancel** (create-order and payment are online-only — no temp-id reconciliation in the gate); **new-order is a thin picker** (flat `menu_items` `/sync` list, no modifier UI — full Menu is Wave 4); `OutboxReplayer` refined to **drop permanent 4xx** (a stale queued transition the order has already passed → 422) and **halt on transport/5xx/429**, so the order state machine is the idempotency guard. iOS slice implemented first (own repo `cheflogik-ios`, branch `wave2-orders-notifications`); the Android mirror is a separate plan once iOS is verified.

---

## Decision 51 — Mobile Location Stamping & Geofenced Login (Per-Branch, audit_log-backed)

**Date decided:** 2026-06-17
**Context:** The native apps (Decision 50) are login/operational apps used on-site. To prove an employee was physically at the branch when they signed in and performed sensitive actions, mobile clients capture device GPS and the backend records/validates it. **Web is unaffected** — location applies to mobile clients only.
**Setting (per-branch, via the existing `SettingsService` branch scope — no branch-table migration):** `mobile.location_required` (bool, default `false`) and `mobile.geofence_radius_m` (int metres, default `150`). Geofence centre = the branch's `address.lat`/`address.lng`.
**Enforcement model (confirmed "both ways"):**
- **`location_required = true` → hard gate.** The mobile app will not function unless the OS location permission is granted and a fix is shared. **Login is proximity-checked**: the server rejects sign-in when the device fix is outside the branch geofence, or when no fix is supplied. Critical actions carry coordinates.
- **`location_required = false` → best-effort.** The app works without location; but if the employee HAS shared a fix, coordinates **and** the computed in-geofence result are still captured on critical actions. Never blocks.
**Geo-stamped actions (login + defined critical set):** OTP login verify, clock-in / clock-out, payments & refunds, order status changes, 86 / stock-out events, inventory adjustments.
**Storage (extend `audit_log`, merged into `create_audit_log_table` — staging not yet verified, GAPS §8):** add `latitude` decimal(10,7) null, `longitude` decimal(10,7) null, `accuracy_m` decimal(8,2) null, `captured_at` timestampTz null, `within_geofence` boolean null, `distance_m` decimal(10,2) null. The login stamp is itself written as an `audit_log` entry. Reuses the write-only / 7-year retention guarantees; single source of truth.
**Transport:** mobile clients send `X-Geo-Latitude`, `X-Geo-Longitude`, `X-Geo-Accuracy`, `X-Geo-Captured-At`, and `X-Client` (`ios`|`android`) headers. A staff-group middleware reads them onto the request; the proximity service (haversine vs branch lat/lng + `geofence_radius_m`) computes `within_geofence` + `distance_m`; the audit writer persists the geo fields. Web requests omit the headers and are exempt. New middleware is **additive** within the staff route group (not a reorder of existing global middleware).
**Privacy/GDPR:** staff GPS is personal data — captured only on mobile, only for the configured actions; iOS `NSLocationWhenInUseUsageDescription` / Android `ACCESS_FINE_LOCATION` consent strings explain why; off-mode best-effort capture still requires the employee's prior OS grant.
**Rationale:** Per-branch (some sites enforce, others don't) chosen over tenant-wide; settings registry avoids schema churn; audit_log reuse keeps one immutable evidence trail; "both ways" lets a tenant record presence opportunistically before mandating it.
