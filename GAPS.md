...
- [x] ~~Async export poll/download flow~~ — ✓ FIXED 2026-06-14 (Wave 8): backend was complete (`POST /analytics/reports/export` → 202 + `job_id`; `GET /exports/{jobId}/status`, gated `analytics.export`) but had **no /web trigger**. Added reusable `web/src/components/analytics/ExportButton.tsx` (self-gates on `analytics.export`; POSTs report_type/branch/from/to, polls status every 2.5 s, auto-opens the 24 h S3 download when ready) wired into all 6 report pages (revenue/dishes/inventory/staff/tax with their date+branch filters; customers dashboard with no range → backend 30-day default). CSV only — backend PDF path falls back to CSV, so no format toggle exposed. en-US `analytics.export_*` keys.
...
- [x] ~~Custom date-range picker (presets only)~~ — ✓ FIXED 2026-06-14: the note was inverted — report pages already had free-range from/to DatePickers but no quick presets. Added shared `DateRangePresets` (Last 7/30/90 days, This month, Last month) to revenue/tax/inventory/revpash/staff; preset applies+reloads immediately, manual edits clear the active chip. en-US labels in `analytics.range_*`.
- [x] ~~Metric alert thresholds UI; custom report builder~~ — ✓ FIXED 2026-06-14 (Wave 9): two net-new features. **Metric alerts (Decision 46)** — new `metric_alerts` table + `MetricAlert` model; staff define metric + operator + threshold (optional branch); `EvaluateMetricAlertsCommand` runs every 15 min over a completed-day branch-local window (yesterday/last_7/last_30), notifies in-app (`NotificationService`) + email (`MetricAlertBreachedMail`) on breach, throttled to once/day per alert via `last_triggered_at`. `MetricAlertController` (CRUD, gated `analytics.configure_alerts`) + `MetricAlertEvaluator` (10 metrics from existing aggregates/report services). `/web` `/analytics/alerts` page + nav tab. **Custom report builder = saved report configurations (Decision 47)** — `SavedReportController` GET/PUT over the `reporting.saved_reports` setting (gated `analytics.custom_reports`); a named {report_type, branch, date-range/preset} layer over existing reports. `/web` `/analytics/saved-reports` page + nav tab with per-row View link + reused `ExportButton`. No new permission slugs (both existed). Tests: `MetricAlertTest`, `SavedReportTest`.
...
- [x] ~~Recurring events UI~~ — ✓ FIXED 2026-06-14 (**Decision 43**): `generateRecurringChildren` existed but was **never wired to a route** and `recurrence_rule` was unvalidated. Added `POST /events/{event}/recurrence` (events.manage) + `EventService::applyRecurrence` (regenerate: clears future non-completed children, stores `{frequency:weekly|monthly, occurrences:1–52}` rule, regenerates). `show()` eager-loads parent/children. No migration/dep. `/web` event-detail **Recurrence tab** (parent: generate/regenerate form + occurrences list; child: link to parent).

### Customers
- [x] ~~Loyalty redemption at POS checkout~~ — ✓ FIXED 2026-06-14: backend already accepted `customer_profile_id` + `loyalty_points_to_redeem` (StoreOrderRequest) and redeemed via `LoyaltyService::redeemPoints` — the order-create form never sent them. Added a `customers.view_basic`-gated loyalty section to `/web` `orders/new.tsx`: debounced `GET /customers?search` customer picker, balance display, and a points-redeem `NumberInput` (multiples of 100, capped at balance). No backend change.
...
### Cross-cutting
- [x] ~~Global search (header input is static)~~ — ✓ FIXED 2026-06-14 (Wave 8, **Decision 45**): net-new `GET /v1/search?q=` (`SearchController`) over customers/orders/reservations/events/menu-items — grouped, top-5 per type, each source gated by its module view permission (`customers.view_basic`/`orders.view`/`reservations.view`/`events.view`/`menu.view`); all tenant-scoped via global scope. `/web` `GlobalSearch.tsx` replaces the static header `SearchInput`: debounced grouped dropdown → navigates to each result's detail route. No migration/dep/new slug.
